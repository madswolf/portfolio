...
The top cap, main housing, bottom cap, and the two gaskets are the components used to enclose the components inside. 
The linear actuator is what drives the expansion and compression by moving the gaskets in relation to each other. \
The two caps block the two gaskets from extending too far and letting water inside. 
We used a Raspberry Pi Pico W for control and reading sensors along with wireless communication.

This makes up a watertight robot taking up 808 ml and weighing a minimum of 528 g.
It can increase its total volume by up to 102.64 ml, and therefore manipulate its own density to a minimum of 0.5798 kg/L and variable maximums based on additional ballast.
...
However, it was not really meant for a general audience when written, so keep that in mind if you decide to read it.
